  ...
  return (
    ...
    >
      ...
      >
        {/* Folder Back */}
        <div
          className="relative w-[110px] h-[85px] transition-all duration-500 rounded-tr-[4px] rounded-br-[4px] rounded-bl-[4px] border-2 border-black"
          style={{ 
            backgroundColor: folderBackColor,
            boxShadow: isOpen ? '0 10px 30px -5px rgba(0,0,0,0.15)' : '0 4px 12px -2px rgba(0,0,0,0.1)'
          }}
        >
          {/* Tab */}
          <div
            className="absolute bottom-full left-0 w-[40px] h-[12px] rounded-t-[4px] border-t-2 border-l-2 border-r-2 border-black"
          ...
          {displayItems.map((item, i) => (
            <motion.div
              ...
              className="absolute left-1/2 flex items-center justify-center overflow-hidden border-2 border-black"
              style={{
                zIndex: 20,
            ...
            >
              {item || (
                <div className="w-full h-full p-2 flex flex-col gap-1.5 opacity-40 text-black">
                  <div className="w-3/4 h-1 bg-current" />
                  <div className="w-1/2 h-1 bg-current" />
                  <div className="w-2/3 h-1 bg-current" />
                </div>
              )}
            </motion.div>
          ))}
          ...
          <motion.div
            ...
            className="absolute inset-0 z-30 origin-bottom border-2 border-black"
            style={{
          ...
          >
             {label && !isOpen && (
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-white font-black text-[10px] tracking-widest uppercase whitespace-nowrap px-2">
                {label}
              </div>
            )}
          </motion.div>
